...
export default function OrderConfirmationClient({ initialOrder, orderId }: Props) {
  ...
  useEffect(() => {
    if (!order && typeof window !== 'undefined') {
      try {
        const saved =
          sessionStorage.getItem(`last_reservation_${orderId}`) ||
          sessionStorage.getItem('last_reservation_latest');
        if (saved) {
  ...
  const pickupDate = order?.pickupDate || new Date().toISOString();
    ...
    <div className="min-h-screen pt-20 pb-16">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
          ...
          <div className="px-6 py-5 space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="text-[#F3ECE0] font-medium">{customerName}</p>
              </div>
              <div>
                <p className="text-[#C4A49E] text-xs mb-0.5">Phone</p>
                <p className="text-[#F3ECE0] font-medium">{customerPhone || 'N/A'}</p>
              </div>
              {customerEmail && (
                <div>
                  <p className="text-[#C4A49E] text-xs mb-0.5">Email</p>
                  <p className="text-[#F3ECE0] font-medium text-xs truncate">{customerEmail}</p>
                </div>
              )}
              <div>
                <p className="text-[#C4A49E] text-xs mb-0.5">Preferred Pickup</p>
                <p className="text-[#F3ECE0] font-medium">{formatDate(pickupDate)}</p>
              </div>
            </div>
            ...
            {total > 0 && (
              <div className="border-t border-[#2a2a2a] pt-3 flex justify-between">
                ...
                <span className="font-display text-xl font-bold text-[#C9A24B]">{formatPrice(total)}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <a
            href={`https://wa.me/919876543210?text=Hi! I have a reservation at Fashion Spot. Code: ${reservationCode}`}
            target="_blank"
